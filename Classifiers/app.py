...
def main():
    if request.method == 'GET':
        result = request.form
        print(result)
        return 'This is a get request from python'
    elif request.method == 'POST':
        print("Starting Classification on Python side...")
        encoder = HTMLEncoder()
        encoder.clear()
        parser = MyHTMLParser()
        parser.clear()
        
        response = Response(request.form)
        handler = DataHandler()
        clf = RFC()
        ...
        predictions, classifier = clf.findbestestimator(training_input, training_output, testing_input, testing_output)
        return clf.test_raw_html(response.retrieve_html(), parser, encoder)
    else:
        return 'None'
